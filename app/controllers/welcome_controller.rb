...
class WelcomeController < ApplicationController
  ...
  def getting_production_data
    ...
    Product.all.each do |product|
      ...
      u ='https://dnlapps.dnlpune.com/DPLPlan/SalesOutbound?product=' + product.product_num.to_s + '&zone='+ zone + '&startDate=' + start_date + '&endDate=' + end_date + '&accessCode=' + token
      url = u
      puts url
      response = HTTParty.get(url)
      @data = JSON.parse(response)
      @data['data'].keys.each do |key|
        ...
        date = @data['data'][key]['date']
        date = date.to_date
        sales = SalesOutbound.where('date =? and product_id =? ', date, product.id).first
        if sales.blank?
          sales = SalesOutbound.new
          sales.product_id = product.id
          sales.date = date
