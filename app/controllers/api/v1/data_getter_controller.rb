class Api::V1::DataGetterController < ApplicationController
	...
	def homedata
	  ...
	  sales_phenol = 	SalesOutbound.where(date:sales_start..sales_end,product:Product.where(name:['Phenol','Hydrated Phenol'])).sum(:metric_tons).round(2)
		...
		inbound_coal_tt = Inbound.where(date: Date.yesterday,product:Product.where(name:'Coal'),logistic_location:LogisticLocation.where(name:'unloading')).sum(:total_tons).round(2)
		inbound_cumene_tt = Inbound.where(date: Date.yesterday,product:Product.where(name:'Cumene'),logistic_location:LogisticLocation.where(name:'unloading')).sum(:total_tons).round(2)
	...
	def sales
		...
		render json:{data: {'zone': [{ 'name' : 'Phenol', 'qty' : sales_phenol, 'north_qty' : phenol_north_zone, 'west_qty' : phenol_west_zone, 'south_qty' : phenol_south_zone, 'east_qty' : phenol_east_zone, 'central_qty' : phenol_central_zone, 'export_qty' : phenol_export_zone},{'name' : 'Acetone', 'qty' : sales_acetone, 'north_qty' : acetone_north_zone, 'west_qty' : acetone_west_zone,
			 'south_qty' : acetone_south_zone, 'east_qty' : acetone_east_zone, 'central_qty' : acetone_central_zone, 'export_qty' : acetone_export_zone}], 'other':[{'name' : 'Heavies', 'qty' : heives_other_zone}]}, success: true, message:""}
